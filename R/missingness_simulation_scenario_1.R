...
for (repetition in c(1:n_rep)) {
  ...
  # data = NULL, num_datasets = NULL, repetitions = NULL, imp_method = NULL
  if (missingness_handling == "CCA") {
    handled_MNAR_dataset <- apply_CCA(MNAR_dataset)
    handled_MCAR_dataset <- apply_CCA(MCAR_dataset)
  } else if (missingness_handling == "naive_MI") {
    handled_MNAR_dataset <- apply_naive_MI(MNAR_dataset, num_datasets = 5, repetitions = 20, imp_method = "pmm")
    handled_MCAR_dataset <- apply_naive_MI(MCAR_dataset, num_datasets = 5, repetitions = 20, imp_method = "pmm")
  }
