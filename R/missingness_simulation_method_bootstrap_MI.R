...
# See BootImpute documentation (https://cran.r-project.org/web/packages/bootImpute/index.html)
impM <- function(inputData, M, repetitions = NULL, imp_method = NULL) {
  capture.output(                                # suppress command line output
    miceImps <- mice::mice(inputData,
...
apply_bootstrap_MI <- function(data = NULL, num_datasets = NULL, repetitions = NULL, imp_method = NULL) {
  imp_data <- NULL
  ...
  imps <- bootImpute::bootImpute(obsdata     = linquad,
                                 impfun      = impM,
                                 nBoot       = 200,
                                 nImp        = num_datasets,
                                 M           = num_datasets,
  ...
  bootImpute::bootImputeAnalyse(imps)
  stop("dev")
  
  return (NULL)
}

run_bootstrap_MI_simulation <- function(n_scenario = NULL,
                                        n_obs      = NULL,
                                        n_rep      = NULL,
  ...
  for (repetition in c(1:n_rep)) {
    ...
    stop("TODO: read up more on bootstrap and finish!")
    
    handled_MNAR_dataset <- apply_bootstrap_MI(data         = MNAR_dataset,
                                               num_datasets = 5,
                                               repetitions  = 20,
                                               imp_method   = "pmm")
    handled_MCAR_dataset <- apply_bootstrap_MI(data         = MCAR_dataset,
                                               num_datasets = 5,
                                               repetitions  = 20,
                                               imp_method   = "pmm")
