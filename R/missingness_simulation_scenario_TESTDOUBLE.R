...
X_CCA_MNAR_dataset <- subset(CCA_MNAR_dataset, select=-c(Y))
Z_CCA_MNAR_dataset <- subset(CCA_MNAR_dataset, select=-c(Y, X))
Y_CCA_MNAR_column  <- subset(CCA_MNAR_dataset, select=c(Y))
X_CCA_MNAR_column  <- subset(CCA_MNAR_dataset, select=c(X))

X_CCA_MCAR_dataset <- subset(CCA_MCAR_dataset, select=-c(Y))
Z_CCA_MCAR_dataset <- subset(CCA_MCAR_dataset, select=-c(Y, X))
Y_CCA_MCAR_column  <- subset(CCA_MCAR_dataset, select=c(Y))
X_CCA_MCAR_column  <- subset(CCA_MCAR_dataset, select=c(X))

# print(head(full_dataset))
# print(head(MNAR_dataset))
# print(head(CCA_MNAR_dataset))
# stop("here")

# fully adjusted model
if (binary_Y) {
  fully_adjusted_MNAR_model <- glm("Y ~ .", data = CCA_MNAR_dataset, family = "binomial")
  fully_adjusted_MCAR_model <- glm("Y ~ .", data = CCA_MCAR_dataset, family = "binomial")
} else {
  fully_adjusted_MNAR_model  <- lm("Y ~ .", data = CCA_MNAR_dataset)
  fully_adjusted_MCAR_model  <- lm("Y ~ .", data = CCA_MCAR_dataset)
}

# unadjusted model
if (binary_Y) {
  unadjusted_MNAR_model <- glm("Y ~ X", data = CCA_MNAR_dataset, family = "binomial")
  unadjusted_MCAR_model <- glm("Y ~ X", data = CCA_MCAR_dataset, family = "binomial")
} else {
  unadjusted_MNAR_model  <- lm("Y ~ X", data = CCA_MNAR_dataset)
  unadjusted_MCAR_model  <- lm("Y ~ X", data = CCA_MCAR_dataset)
}

# LASSO (outcome Y)
if (binary_Y) {
  cv_lasso_MNAR_model <- cv.glmnet(x = data.matrix(X_CCA_MNAR_dataset), y = data.matrix(Y_CCA_MNAR_column), family = 'binomial', alpha=1)
  lambda              <- cv_lasso_MNAR_model$lambda.min
  lasso_MNAR_model    <- glmnet(x = data.matrix(X_CCA_MNAR_dataset), y = data.matrix(Y_MCCA_NAR_column), family = 'binomial', alpha=1, lambda=lambda)
  ...
  two_step_lasso_MNAR_model <- glm(make_model_formula(vars_selected = lasso_MNAR_vars_selected), data = CCA_MNAR_dataset, family = "binomial")
  
  cv_lasso_MCAR_model <- cv.glmnet(x = data.matrix(X_CCA_MCAR_dataset), y = data.matrix(Y_CCA_MCAR_column), family = 'binomial', alpha=1)
  lambda              <- cv_lasso_MCAR_model$lambda.min
  lasso_MCAR_model    <- glmnet(x = data.matrix(X_CCA_MCAR_dataset), y = data.matrix(Y_CCA_MCAR_column), family = 'binomial', alpha=1, lambda=lambda)
  ...
  two_step_lasso_MCAR_model <- glm(make_model_formula(vars_selected = lasso_MCAR_vars_selected), data = CCA_MCAR_dataset, family = "binomial")
  
} else {
  cv_lasso_MNAR_model <- cv.glmnet(x = data.matrix(X_CCA_MNAR_dataset), y = data.matrix(Y_CCA_MNAR_column), alpha=1)
  lambda              <- cv_lasso_MNAR_model$lambda.min
  lasso_MNAR_model    <- glmnet(x = data.matrix(X_CCA_MNAR_dataset), y = data.matrix(Y_CCA_MNAR_column), alpha=1, lambda=lambda)
  
  lasso_MNAR_coefs          <- as.vector(lasso_MNAR_model$beta)
  names(lasso_MNAR_coefs)   <- rownames(lasso_MNAR_model$beta)
  lasso_MNAR_vars_selected  <- union(c('X'), names(lasso_MNAR_coefs[lasso_MNAR_coefs != 0.0])) # always select X
  lasso_MNAR_vars_selected  <- lasso_MNAR_vars_selected[lasso_MNAR_vars_selected != "(Intercept)"] # exclude intercept
  two_step_lasso_MNAR_model <- lm(make_model_formula(vars_selected = lasso_MNAR_vars_selected), data = CCA_MNAR_dataset)
  
  cv_lasso_MCAR_model <- cv.glmnet(x = data.matrix(X_CCA_MCAR_dataset), y = data.matrix(Y_CCA_MCAR_column), alpha=1)
  lambda              <- cv_lasso_MCAR_model$lambda.min
  lasso_MCAR_model    <- glmnet(x = data.matrix(X_CCA_MCAR_dataset), y = data.matrix(Y_CCA_MCAR_column), alpha=1, lambda=lambda)
  
  lasso_MCAR_coefs          <- as.vector(lasso_MCAR_model$beta)
  names(lasso_MCAR_coefs)   <- rownames(lasso_MCAR_model$beta)
  lasso_MCAR_vars_selected  <- union(c('X'), names(lasso_MCAR_coefs[lasso_MCAR_coefs != 0.0])) # always select X
  lasso_MCAR_vars_selected  <- lasso_MCAR_vars_selected[lasso_MCAR_vars_selected != "(Intercept)"] # exclude intercept
  two_step_lasso_MCAR_model <- lm(make_model_formula(vars_selected = lasso_MCAR_vars_selected), data = CCA_MCAR_dataset)
}

MNAR_coefs["fully_adjusted", , 1] <- fill_in_blanks(fully_adjusted_MNAR_model$coefficients, var_names_except_Y_with_intercept)
MNAR_coefs["unadjusted", , 1]     <- fill_in_blanks(unadjusted_MNAR_model$coefficients, var_names_except_Y_with_intercept)
MNAR_coefs["two_step_lasso", , 1] <- fill_in_blanks(two_step_lasso_MNAR_model$coefficients, var_names_except_Y_with_intercept)

MCAR_coefs["fully_adjusted", , 1] <- fill_in_blanks(fully_adjusted_MCAR_model$coefficients, var_names_except_Y_with_intercept)
MCAR_coefs["unadjusted", , 1]     <- fill_in_blanks(unadjusted_MCAR_model$coefficients, var_names_except_Y_with_intercept)
MCAR_coefs["two_step_lasso", , 1] <- fill_in_blanks(two_step_lasso_MCAR_model$coefficients, var_names_except_Y_with_intercept)

MNAR_results["fully_adjusted", "causal_true_value", 1]      <- causal
MNAR_results["fully_adjusted", "causal_estimate", 1]        <- unname(fully_adjusted_MNAR_model$coefficients['X'])
MNAR_results["fully_adjusted", "causal_bias", 1]            <- (unname(fully_adjusted_MNAR_model$coefficients['X']) - causal)
MNAR_results["fully_adjusted", "causal_bias_proportion", 1] <- ((unname(fully_adjusted_MNAR_model$coefficients['X']) - causal)/causal)
MNAR_results["fully_adjusted", "causal_coverage", 1]        <- estimate_within_CI(model = fully_adjusted_MNAR_model)
MNAR_results["fully_adjusted", "open_paths", 1]             <- num_total_conf
MNAR_results["fully_adjusted", "blocked_paths", 1]          <- num_meas_conf
MNAR_results["fully_adjusted", "proportion_paths", 1]       <- num_meas_conf / num_total_conf
MNAR_results["fully_adjusted", "empirical_SE", 1]           <- NaN
MNAR_results["fully_adjusted", "model_SE", 1]               <- (coef(summary(fully_adjusted_MNAR_model))[, "Std. Error"])['X']

MNAR_results["unadjusted", "causal_true_value", 1]      <- causal
MNAR_results["unadjusted", "causal_estimate", 1]        <- unname(unadjusted_MNAR_model$coefficients['X'])
MNAR_results["unadjusted", "causal_bias", 1]            <- (unname(unadjusted_MNAR_model$coefficients['X']) - causal)
MNAR_results["unadjusted", "causal_bias_proportion", 1] <- ((unname(unadjusted_MNAR_model$coefficients['X']) - causal)/causal)
MNAR_results["unadjusted", "causal_coverage", 1]        <- estimate_within_CI(model = unadjusted_MNAR_model)
MNAR_results["unadjusted", "open_paths", 1]             <- num_total_conf
MNAR_results["unadjusted", "blocked_paths", 1]          <- 0.0
MNAR_results["unadjusted", "proportion_paths", 1]       <- 0.0 / num_total_conf
MNAR_results["unadjusted", "empirical_SE", 1]           <- NaN
MNAR_results["unadjusted", "model_SE", 1]               <- (coef(summary(unadjusted_MNAR_model))[, "Std. Error"])['X']

MNAR_results["two_step_lasso", "causal_true_value", 1]      <- causal
MNAR_results["two_step_lasso", "causal_estimate", 1]        <- unname(two_step_lasso_MNAR_model$coefficients['X'])
MNAR_results["two_step_lasso", "causal_bias", 1]            <- (unname(two_step_lasso_MNAR_model$coefficients['X']) - causal)
MNAR_results["two_step_lasso", "causal_bias_proportion", 1] <- ((unname(two_step_lasso_MNAR_model$coefficients['X']) - causal)/causal)
MNAR_results["two_step_lasso", "causal_coverage", 1]        <- estimate_within_CI(model = two_step_lasso_MNAR_model)
MNAR_results["two_step_lasso", "open_paths", 1]             <- num_total_conf
MNAR_results["two_step_lasso", "blocked_paths", 1]          <- length(lasso_MNAR_vars_selected[lasso_MNAR_vars_selected != 'X'])
MNAR_results["two_step_lasso", "proportion_paths", 1]       <- length(lasso_MNAR_vars_selected[lasso_MNAR_vars_selected != 'X']) / num_total_conf
MNAR_results["two_step_lasso", "empirical_SE", 1]           <- NaN
MNAR_results["two_step_lasso", "model_SE", 1]               <- (coef(summary(two_step_lasso_MNAR_model))[, "Std. Error"])['X']

MCAR_results["fully_adjusted", "causal_true_value", 1]      <- causal
MCAR_results["fully_adjusted", "causal_estimate", 1]        <- unname(fully_adjusted_MCAR_model$coefficients['X'])
MCAR_results["fully_adjusted", "causal_bias", 1]            <- (unname(fully_adjusted_MCAR_model$coefficients['X']) - causal)
MCAR_results["fully_adjusted", "causal_bias_proportion", 1] <- ((unname(fully_adjusted_MCAR_model$coefficients['X']) - causal)/causal)
MCAR_results["fully_adjusted", "causal_coverage", 1]        <- estimate_within_CI(model = fully_adjusted_MCAR_model)
MCAR_results["fully_adjusted", "open_paths", 1]             <- num_total_conf
MCAR_results["fully_adjusted", "blocked_paths", 1]          <- num_meas_conf
MCAR_results["fully_adjusted", "proportion_paths", 1]       <- num_meas_conf / num_total_conf
MCAR_results["fully_adjusted", "empirical_SE", 1]           <- NaN
MCAR_results["fully_adjusted", "model_SE", 1]               <- (coef(summary(fully_adjusted_MCAR_model))[, "Std. Error"])['X']

MCAR_results["unadjusted", "causal_true_value", 1]      <- causal
MCAR_results["unadjusted", "causal_estimate", 1]        <- unname(unadjusted_MCAR_model$coefficients['X'])
MCAR_results["unadjusted", "causal_bias", 1]            <- (unname(unadjusted_MCAR_model$coefficients['X']) - causal)
MCAR_results["unadjusted", "causal_bias_proportion", 1] <- ((unname(unadjusted_MCAR_model$coefficients['X']) - causal)/causal)
MCAR_results["unadjusted", "causal_coverage", 1]        <- estimate_within_CI(model = unadjusted_MCAR_model)
MCAR_results["unadjusted", "open_paths", 1]             <- num_total_conf
MCAR_results["unadjusted", "blocked_paths", 1]          <- 0.0
MCAR_results["unadjusted", "proportion_paths", 1]       <- 0.0 / num_total_conf
MCAR_results["unadjusted", "empirical_SE", 1]           <- NaN
MCAR_results["unadjusted", "model_SE", 1]               <- (coef(summary(unadjusted_MCAR_model))[, "Std. Error"])['X']

MCAR_results["two_step_lasso", "causal_true_value", 1]      <- causal
MCAR_results["two_step_lasso", "causal_estimate", 1]        <- unname(two_step_lasso_MCAR_model$coefficients['X'])
MCAR_results["two_step_lasso", "causal_bias", 1]            <- (unname(two_step_lasso_MCAR_model$coefficients['X']) - causal)
MCAR_results["two_step_lasso", "causal_bias_proportion", 1] <- ((unname(two_step_lasso_MCAR_model$coefficients['X']) - causal)/causal)
MCAR_results["two_step_lasso", "causal_coverage", 1]        <- estimate_within_CI(model = two_step_lasso_MCAR_model)
MCAR_results["two_step_lasso", "open_paths", 1]             <- num_total_conf
MCAR_results["two_step_lasso", "blocked_paths", 1]          <- length(lasso_MCAR_vars_selected[lasso_MCAR_vars_selected != 'X'])
MCAR_results["two_step_lasso", "proportion_paths", 1]       <- length(lasso_MCAR_vars_selected[lasso_MCAR_vars_selected != 'X']) / num_total_conf
MCAR_results["two_step_lasso", "empirical_SE", 1]           <- NaN
MCAR_results["two_step_lasso", "model_SE", 1]               <- (coef(summary(two_step_lasso_MCAR_model))[, "Std. Error"])['X']
...
message("\n\n\n\n ***** Confounders *****")
print("Subgroup X Low, Y low")
print(paste('Z', c(1:8), sep=''))
print("Subgroup X Low, Y high")
print(paste('Z', c(9:16), sep=''))
print("Subgroup X High, Y low")
print(paste('Z', c(17:24), sep=''))
print("Subgroup X High, Y high")
print(paste('Z', c(25:32), sep=''))
print("Unmeasured confounders")
print(paste('Z', c(1, 9, 17, 25), sep=''))
...
# Save to file
id_string <- paste("sim_scenario_", n_scenario, sep='')
