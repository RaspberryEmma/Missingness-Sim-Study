...
apply_MLMI <- function(data = NULL, vars_to_censor = NULL) {
  num_cat <- sum(as.numeric(binary_X) + as.numeric(binary_Y) + num_total_conf*as.numeric(binary_Z))
  
  if (num_cat > 0) {
    imp_data <- mixImp(obsData = data,
                       nCat    = sum(as.numeric(binary_X) + as.numeric(binary_Y) + num_total_conf*as.numeric(binary_Z)),
                       M       = 5,
                       pd      = FALSE,
                       rseed   = NULL)
  } else {
    imp_data <- normImp(obsData = data,
                        M       = 5,
                        pd      = FALSE,
                        rseed   = NULL)
  }
  ...
  View(imp_data)
  stop("dev - MLMI.  FIXES: CHECK _missing everywhere, decide how to think about 5 datasets (copy naive?)")
  
  return (data)
}


run_MLMI_simulation <- function(n_scenario = NULL,
                                n_obs      = NULL,
                                n_rep      = NULL,
  ...
  for (repetition in c(1:n_rep)) {
    ...
    # apply dichotomize
    handled_MNAR_dataset <- apply_MLMI(data = MNAR_dataset, vars_to_censor = vars_to_censor)
    handled_MCAR_dataset <- apply_MLMI(data = MCAR_dataset, vars_to_censor = vars_to_censor)
